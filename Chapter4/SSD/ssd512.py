...
class SSD512(nn.Module):
    ...
    def __init__(self, label_num, vgg_path="./vgg16n.pth"):
        ...
        self.out_chan = [512, 1024, 512, 256, 256, 256, 256]
        self.loc = []
        self.conf = []
        ...
        self.out_layers = [self.block8, self.block9, self.block10, self.block11, self.block12]
        self._init_weights()
    
    def _init_weights(self):
        
        layers = [
            self.block8, self.block9, self.block10, self.block11, self.block12,
            *self.loc, *self.conf]
    ...
    def forward(self, data):
        ...
        src = [self.l2norm4(layer4), layer7]
        
        x = layer7
        for lout in self.out_layers:
            x = lout(x)
            src.append(x)

        #src = [self.l2norm4(layer4), layer7, layer8, layer9, layer10, layer11, layer12]
